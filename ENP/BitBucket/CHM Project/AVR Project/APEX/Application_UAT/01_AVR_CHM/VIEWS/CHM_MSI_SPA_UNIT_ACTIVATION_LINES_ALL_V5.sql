  ...
  'MAIN_SKU' item_source,
   0 priority,
    ...
    csh.spa_total_price_currency SPA_CURRENCY,
		...
		--csl.unit_rebate_amount REBATE_AMOUNT,
	...
	csh.tier4_max,
	csh.tier5_max	
	FROM CHM_MSI_SPA_UNIT_ACTIVATIONS CSL,
	--chm_msi_spa_unit_activation_incentives  CSL,
...
SELECT 
  -- when spa alternate sku with price is provided in lookup
    ...
    --CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES  CSL,
	CHM_MSI_SPA_UNIT_ACTIVATIONS  CSL,
    CHM_MSI_SPA_HEADER CSH
	WHERE  1=1
 ...
 AND UPPER(NVL(csl.status,'xx'))='APPROVED'
  AND caa.type = 'SKU'
	...
	AND (caa.spa_code,caa.enphase_code) NOT IN
                    ...
                    FROM CHM_MSI_SPA_UNIT_ACTIVATIONS  CSL1,
...
SELECT DISTINCT
 ...
 NOT IN (SELECT csh1.spa_number,  csl1.product 
				FROM  --CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES  CSL1,
						CHM_MSI_SPA_UNIT_ACTIVATIONS CSL1,
				  		CHM_MSI_SPA_HEADER CSH1
					WHERE
