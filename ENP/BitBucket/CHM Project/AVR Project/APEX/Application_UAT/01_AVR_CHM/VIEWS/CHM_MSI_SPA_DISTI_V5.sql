  ...
  SELECT 'MAIN_DISTI' disti_source, 
	CSH.SPA_NUMBER,			substr(csam.ORACLE_CUSTOMER_NUMBER, 1, 
	 ...
	 FROM 	CHM_MSI_SPA_HEADER csh
			,CHM_MSI_SPA_DISTRIBUTOR csd
			,CHM_SFDC_ACCOUNT_MASTER csam
	 WHERE 	
			--csh.spa_number=csd.spa
			csh.id=csd.spa
		AND	csd.distributor_sfdc_account_id=csam.sfdc_account_id
		-- AND UPPER(NVL(csh.is_deleted,'FALSE'))='FALSE'
		 AND UPPER(NVL(csd.is_deleted,'FALSE'))='FALSE'
		 AND UPPER(NVL(csd.spa_status,'xx'))='APPROVED'
;
