  ...
  CREATE TABLE "CHM_MSI_SPA_UNIT_ACTIVATIONS" 
   (	"CHM_MSI_SPA_UNIT_ACTIVATION_ID" NUMBER DEFAULT CHM.CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES_SEQ.NEXTVAL, 
	"ID" VARCHAR2(4000), 
	"MIN_QUANTITY" NUMBER, 
	...
	"PER_UNIT_INCENTIVE" NUMBER NOT NULL ENABLE, 
	"PRODUCT" VARCHAR2(4000), 
	"UNIQUE_KEY" VARCHAR2(4000), 
	"SPA" VARCHAR2(4000) NOT NULL ENABLE, 
	 CONSTRAINT "CHM_MSI_SPA_UNIT_ACTIVATIONS_PK" PRIMARY KEY ("CHM_MSI_SPA_UNIT_ACTIVATION_ID")
  USING INDEX  ENABLE
   ) ;

  CREATE INDEX "CHM_MSI_SPA_UNIT_ACTIVATIONS_ID_IDX" ON "CHM_MSI_SPA_UNIT_ACTIVATIONS" ("ID") 
  ;

  CREATE INDEX "CHM_MSI_SPA_UNIT_ACT_SPA_IDX" ON "CHM_MSI_SPA_UNIT_ACTIVATIONS" ("SPA") 
  ;

  CREATE OR REPLACE EDITIONABLE TRIGGER "AIU_CHM_MSI_SPA_UNIT_ACTIVATION_TRG" AFTER
    INSERT OR UPDATE OR DELETE ON "CHM_MSI_SPA_UNIT_ACTIVATIONS"
    FOR EACH ROW
BEGIN
    IF inserting THEN
        INSERT INTO CHM_MSI_SPA_UNIT_ACTIVATIONS_AUDIT (
            chm_msi_spa_unit_activation_audit_id,
            ...
            chm_msi_spa_unit_activation_incentives_audit_seq.NEXTVAL,
            :new.chm_msi_spa_unit_activation_id,
            :new.ID,
            :new.min_quantity,
            :new.status,
    ...
    ELSIF updating THEN
        INSERT INTO CHM_MSI_SPA_UNIT_ACTIVATIONS_AUDIT (
            ...
            chm_msi_spa_unit_activation_incentives_audit_seq.NEXTVAL,
            :old.chm_msi_spa_unit_activation_id,
            :old.id,
            :new.id,
            :old.min_quantity,
            :new.min_quantity,
...
END;
/
ALTER TRIGGER "AIU_CHM_MSI_SPA_UNIT_ACTIVATION_TRG" ENABLE;
  CREATE OR REPLACE EDITIONABLE TRIGGER "BIU_CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES_TRG" 
  BEFORE INSERT OR UPDATE ON "CHM_MSI_SPA_UNIT_ACTIVATIONS"
  FOR EACH ROW
   BEGIN
      IF INSERTING THEN
        IF :new.CHM_MSI_SPA_UNIT_ACTIVATION_ID  IS NULL THEN
            :new.CHM_MSI_SPA_UNIT_ACTIVATION_ID   := CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES_SEQ.NEXTVAL;
        END IF;
      ...
      ELSIF updating THEN
       ...
       EXCEPTION
  WHEN OTHERS THEN
    chm_msi_debug_api.log_msg(:new.CHM_MSI_SPA_UNIT_ACTIVATION_ID || ', ' || :new.ID, 'BIU_CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES_TRG', SQLERRM);
    END;

/
ALTER TRIGGER "BIU_CHM_MSI_SPA_UNIT_ACTIVATION_INCENTIVES_TRG" ENABLE;