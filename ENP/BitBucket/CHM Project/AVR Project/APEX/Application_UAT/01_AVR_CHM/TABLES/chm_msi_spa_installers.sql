  ...
  CREATE TABLE "CHM_MSI_SPA_INSTALLERS" 
   (	"CHM_MSI_SPA_INSTALLER_ID" NUMBER DEFAULT CHM.CHM_MSI_SPA_INSTALLERS_SEQ.NEXTVAL, 
	"ID" VARCHAR2(4000), 
	...
	"INSTALLER_ID" VARCHAR2(4000) NOT NULL ENABLE, 
	"SPA" VARCHAR2(4000) NOT NULL ENABLE, 
	 CONSTRAINT "CHM_MSI_SPA_INSTALLERS_PK" PRIMARY KEY ("CHM_MSI_SPA_INSTALLER_ID")
  USING INDEX  ENABLE
   ) ;

  CREATE INDEX "CHM_MSI_SPA_INSTALLERS_ID_IDX" ON "CHM_MSI_SPA_INSTALLERS" ("ID") 
  ;

  CREATE INDEX "CHM_MSI_SPA_INSTALLERS_SPA_IDX" ON "CHM_MSI_SPA_INSTALLERS" ("SPA") 
  ;

  CREATE OR REPLACE EDITIONABLE TRIGGER "BIU_CHM_MSI_SPA_INSTALLERS_TRG" 
  BEFORE INSERT OR UPDATE ON CHM_MSI_SPA_INSTALLERS
  FOR EACH ROW
   BEGIN
      IF INSERTING THEN
        IF :new.CHM_MSI_SPA_INSTALLER_ID  IS NULL THEN
            :new.CHM_MSI_SPA_INSTALLER_ID   := CHM_MSI_SPA_INSTALLERS_SEQ.NEXTVAL;
        END IF;
      ...
      ELSIF updating THEN
       ...
       EXCEPTION
  WHEN OTHERS THEN
    chm_msi_debug_api.log_msg(:new.CHM_MSI_SPA_INSTALLER_ID || ', ' || :new.ID, 'BIU_CHM_MSI_SPA_INSTALLERS_TRG', SQLERRM);
    END;

/
ALTER TRIGGER "BIU_CHM_MSI_SPA_INSTALLERS_TRG" ENABLE;
  CREATE OR REPLACE EDITIONABLE TRIGGER "AIU_CHM_MSI_SPA_INSTALLERS_TRG" AFTER
    INSERT OR UPDATE OR DELETE ON chm_msi_spa_installers
    FOR EACH ROW
BEGIN
    IF inserting THEN
        INSERT INTO chm_msi_spa_installers_audit (
            chm_msi_spa_installer_audit_id,
            chm_msi_spa_installer_id,
            new_id,
            new_installer_enlighten_id,
            new_status,
            ...
            chm_msi_spa_installers_audit_seq.NEXTVAL,
            :new.chm_msi_spa_installer_id,
            :new.id,
            :new.installer_enlighten_id,
            :new.status,
    ...
    ELSIF updating THEN
        INSERT INTO chm_msi_spa_installers_audit (
            ...
            chm_msi_spa_installers_audit_seq.NEXTVAL,
            :old.chm_msi_spa_installer_id,
            :old.id,
            :new.id,
            :old.installer_enlighten_id,
            :new.installer_enlighten_id,
            :old.status,
            :new.status,
...
END;
/
ALTER TRIGGER "AIU_CHM_MSI_SPA_INSTALLERS_TRG" ENABLE;