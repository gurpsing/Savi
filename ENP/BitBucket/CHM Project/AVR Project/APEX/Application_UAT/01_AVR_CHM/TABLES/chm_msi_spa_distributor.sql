  ...
  CREATE TABLE "CHM_MSI_SPA_DISTRIBUTOR" 
   (	"CHM_MSI_SPA_DISTRIBUTOR_ID" NUMBER DEFAULT CHM.CHM_MSI_SPA_DISTRIBUTOR_SEQ.NEXTVAL, 
	"ID" VARCHAR2(4000), 
	...
	"DISTRIBUTOR_SFDC_ACCOUNT_ID" VARCHAR2(4000) NOT NULL ENABLE, 
	 CONSTRAINT "CHM_MSI_SPA_DISTRIBUTOR_PK" PRIMARY KEY ("CHM_MSI_SPA_DISTRIBUTOR_ID")
  USING INDEX  ENABLE
   ) ;

  CREATE INDEX "CHM_MSI_SPA_DISTRIBUTOR_SPA_IDX" ON "CHM_MSI_SPA_DISTRIBUTOR" ("SPA") 
  ;

  CREATE INDEX "DISTI_ID_IDX" ON "CHM_MSI_SPA_DISTRIBUTOR" ("ID") 
  ;

  CREATE OR REPLACE EDITIONABLE TRIGGER "AIU_CHM_MSI_SPA_DISTRIBUTOR_TRG" AFTER
    INSERT OR UPDATE OR DELETE ON chm_msi_spa_distributor
    FOR EACH ROW
BEGIN
    IF inserting THEN
        INSERT INTO chm_msi_spa_distributor_audit (
            CHM_MSI_SPA_DISTRIBUTOR_AUDIT_ID,
			...
			IP_ADDRESS,       
			USER_AGENT,
            NEW_DISTRIBUTOR_SFDC_ACCOUNT_ID,
			NEW_IS_DELETED,
			NEW_NAME,
			...
			NEW_SPA,
			NEW_SPA_AND_OPPORTUNITIES,
			NEW_LINE_STATUS,
			NEW_ORACLE_CUSTOMER_NUMBER
        ) VALUES (
            chm_msi_spa_distributor_audit_seq.NEXTVAL,
            :new.CHM_MSI_SPA_DISTRIBUTOR_ID,
            :old.ID,
            :new.ID,
            :old.spa_status,
            :new.spa_status,
            ...
            :new.DISTRIBUTOR_SFDC_ACCOUNT_ID,
			:new.IS_DELETED,
			:new.NAME,
			...
			:new.SPA,
			:new.SPA_AND_OPPORTUNITIES,
			:new.LINE_STATUS,
			:new.ORACLE_CUSTOMER_NUMBER
        );
    ...
    ELSIF updating THEN
        INSERT INTO chm_msi_spa_distributor_audit (
			...
			IP_ADDRESS,       
			USER_AGENT,
            OLD_DISTRIBUTOR_SFDC_ACCOUNT_ID,
            NEW_DISTRIBUTOR_SFDC_ACCOUNT_ID,
			OLD_IS_DELETED,
			NEW_IS_DELETED,
			OLD_NAME,
			...
			OLD_SPA,
			NEW_SPA,
			OLD_SPA_AND_OPPORTUNITIES,
			NEW_SPA_AND_OPPORTUNITIES,
			OLD_LINE_STATUS,
			NEW_LINE_STATUS,
			OLD_ORACLE_CUSTOMER_NUMBER,
			NEW_ORACLE_CUSTOMER_NUMBER
        ) VALUES (
            chm_msi_spa_distributor_audit_seq.NEXTVAL,
            :old.CHM_MSI_SPA_DISTRIBUTOR_ID,
            :old.ID,
            :new.ID,
            :old.spa_status,
            :new.spa_status,
			...
			NVL(:NEW.IP_ADDRESS,CHM_UTIL_PKG.GETIPADDRESS()),
			NVL(:NEW.USER_AGENT,CHM_UTIL_PKG.GETUSERAGENT()),
            :old.DISTRIBUTOR_SFDC_ACCOUNT_ID,
            :new.DISTRIBUTOR_SFDC_ACCOUNT_ID,
			:old.IS_DELETED,
			:new.IS_DELETED,
			:old.NAME,
			...
			:old.SPA,
			:new.SPA,
			:old.SPA_AND_OPPORTUNITIES,
			:new.SPA_AND_OPPORTUNITIES,
			:old.LINE_STATUS,
			:new.LINE_STATUS,
			:old.ORACLE_CUSTOMER_NUMBER,
			:new.ORACLE_CUSTOMER_NUMBER
        );
...
END;
/
ALTER TRIGGER "AIU_CHM_MSI_SPA_DISTRIBUTOR_TRG" ENABLE;
  CREATE OR REPLACE EDITIONABLE TRIGGER "BIU_CHM_MSI_SPA_DISTRIBUTOR_TRG" 
  BEFORE INSERT OR UPDATE ON CHM_MSI_SPA_DISTRIBUTOR
  FOR EACH ROW
   BEGIN
      IF INSERTING THEN
        IF :new.CHM_MSI_SPA_DISTRIBUTOR_ID  IS NULL THEN
            :new.CHM_MSI_SPA_DISTRIBUTOR_ID   := CHM_MSI_SPA_DISTRIBUTOR_SEQ.NEXTVAL;
        END IF;
      ...
      ELSIF updating THEN
       ...
       EXCEPTION
  WHEN OTHERS THEN
    chm_msi_debug_api.log_msg(:new.ID || ', ' || :new.ID, 'BIU_CHM_MSI_SPA_DISTRIBUTOR_TRG', SQLERRM);
    END;
/
ALTER TRIGGER "BIU_CHM_MSI_SPA_DISTRIBUTOR_TRG" ENABLE;