  ...
  CREATE TABLE "CHM_MSI_CLAIM_REQUEST_APPROVAL" 
   (	"REQ_APPROVAL_ID" NUMBER, 
	"MSI_CLAIM_REQ_ID" NUMBER, 
	"APPROVAL_LEVEL" NUMBER, 
	"APPROVAL_ROLE" VARCHAR2(4000), 
	"PRIMARY_APPROVER" VARCHAR2(4000), 
	"PR_APPROVAL_STATUS" VARCHAR2(4000), 
	"PR_APPROVAL_REMARKS" VARCHAR2(4000), 
	"PR_APPROVAL_DATE" TIMESTAMP (6) WITH TIME ZONE, 
	"BACKUP_APPROVER" VARCHAR2(4000), 
	"BK_APPROVAL_STATUS" VARCHAR2(4000), 
	"BK_APPROVAL_REMARKS" VARCHAR2(4000), 
	"BK_APPROVAL_DATE" TIMESTAMP (6) WITH TIME ZONE, 
	...
	"USER_AGENT" VARCHAR2(4000), 
	 CONSTRAINT "CHM_MSI_CLAIM_REQUEST_APPROVAL_PK" PRIMARY KEY ("REQ_APPROVAL_ID")
  USING INDEX  ENABLE
   ) ;

  CREATE INDEX "CHM_MSI_CLAIM_REQUEST_APPROVAL_CLAIM_REQ_ID_IDX" ON "CHM_MSI_CLAIM_REQUEST_APPROVAL" ("MSI_CLAIM_REQ_ID") 
  ;

  CREATE OR REPLACE EDITIONABLE TRIGGER "BIU_CHM_MSI_CLAIM_REQUEST_APPROVAL" BEFORE
    INSERT OR UPDATE ON CHM_MSI_CLAIM_REQUEST_APPROVAL
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
        IF :NEW.REQ_APPROVAL_ID IS NULL THEN
            :NEW.REQ_APPROVAL_ID := CHM_MSI_CLAIM_REQUEST_APPROVAL_SEQ.NEXTVAL;
        END IF;
...
/
ALTER TRIGGER "BIU_CHM_MSI_CLAIM_REQUEST_APPROVAL" ENABLE;