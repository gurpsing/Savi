  ...
  CREATE TABLE "CHM_MSI_SYS_SIZE_INCENTIVE_CHM" 
   (	"CHM_MSI_SPA_SYS_SIZE_DETAIL_ID" NUMBER DEFAULT CHM.CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVES_DETAIL_SEQ.NEXTVAL, 
	"ID" VARCHAR2(4000), 
	...
	"TIER_INCENTIVE" NUMBER NOT NULL ENABLE, 
	"PRODUCT" VARCHAR2(4000), 
	"STATUS" VARCHAR2(4000) NOT NULL ENABLE, 
	"SYS_SIZE_INCENTIVE_ID" VARCHAR2(4000), 
	 CONSTRAINT "CHM_MSI_SYS_SIZE_INCENTIVE_CHM_PK" PRIMARY KEY ("CHM_MSI_SPA_SYS_SIZE_DETAIL_ID")
  USING INDEX  ENABLE
   ) ;

  CREATE INDEX "CHM_MSI_SYS_SIZE_CHM_SPA_IDX" ON "CHM_MSI_SYS_SIZE_INCENTIVE_CHM" ("SPA") 
  ;

  CREATE INDEX "CHM_MSI_SYS_SIZE_INCENTIVE_CHM_ID_IDX" ON "CHM_MSI_SYS_SIZE_INCENTIVE_CHM" ("ID") 
  ;

  CREATE OR REPLACE EDITIONABLE TRIGGER "BIU_CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVE_DETAIL_TRG" 
  BEFORE INSERT OR UPDATE ON "CHM_MSI_SYS_SIZE_INCENTIVE_CHM"
  FOR EACH ROW
   BEGIN
      IF INSERTING THEN
        IF :new.CHM_MSI_SPA_SYS_SIZE_DETAIL_ID  IS NULL THEN
            :new.CHM_MSI_SPA_SYS_SIZE_DETAIL_ID   := CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVES_DETAIL_SEQ.NEXTVAL;
        END IF;
      ...
      ELSIF updating THEN
       ...
       EXCEPTION
  WHEN OTHERS THEN
    chm_msi_debug_api.log_msg(:new.CHM_MSI_SPA_SYS_SIZE_DETAIL_ID || ', ' || :new.ID, 'BIU_CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVE_DETAIL_TRG', SQLERRM);
    END;

/
ALTER TRIGGER "BIU_CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVE_DETAIL_TRG" ENABLE;
  CREATE OR REPLACE EDITIONABLE TRIGGER "AIU_CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVE_DETAIL_TRG" AFTER
    INSERT OR UPDATE OR DELETE ON "CHM_MSI_SYS_SIZE_INCENTIVE_CHM"
    FOR EACH ROW
BEGIN
    IF inserting THEN
        INSERT INTO CHM_MSI_SYS_SIZE_INCENTIVE_CHM_AUDIT (
            chm_msi_spa_sys_size_detail_audit_id,
			...
			NEW_PRODUCT,
			NEW_STATUS,
			NEW_SYS_SIZE_INCENTIVE_ID			
        ) VALUES (
            chm_msi_spa_system_size_incentive_detail_audit_seq.NEXTVAL,
            :new.chm_msi_spa_sys_size_detail_id,
            :old.id,
            :new.id,
            :old.tier_name,
            :new.tier_name,
    ...
    ELSIF updating THEN
        INSERT INTO CHM_MSI_SYS_SIZE_INCENTIVE_CHM_AUDIT (
			...
			OLD_STATUS,
			NEW_STATUS,
			OLD_SYS_SIZE_INCENTIVE_ID,
			NEW_SYS_SIZE_INCENTIVE_ID
        ) VALUES (
            chm_msi_spa_system_size_incentive_detail_audit_seq.NEXTVAL,
            :old.chm_msi_spa_sys_size_detail_id,
            :old.id,
            :new.id,
            :old.tier_name,
            :new.tier_name,
...
END;
/
ALTER TRIGGER "AIU_CHM_MSI_SPA_SYSTEM_SIZE_INCENTIVE_DETAIL_TRG" ENABLE;